...
absl::Status Autotuner::Autotune(HloModule* module,
  ...
  auto GetKvStoreKeyForShard = [&](int shard_id) -> std::string {
  ...
  const size_t bucket_size =
      (all_instructions_by_fingerprint.size() + total_shards - 1) /
      total_shards;
  ...
  if (instructions_by_fingerprint.empty()) {
    VLOG(1) << "No instructions to autotune for this shard.";
    return absl::OkStatus();
  }

  // 3. Autotune the instructions for this shard and update the local cache.
  VLOG(1) << "Finding configs for " << instructions_by_fingerprint.size()
          << " unique instructions for shard " << my_shard_index;
  ...
  const std::string local_key = GetKvStoreKeyForShard(my_shard_index);
  TF_ASSIGN_OR_RETURN(std::string local_results,
  ...
  for (int i = 0; i < total_shards; ++i) {
    ...
    const std::string remote_key = GetKvStoreKeyForShard(i);
